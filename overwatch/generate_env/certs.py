"""Self-signed certificate generation for Overwatch platform services."""
...
def _build_ca(
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Overwatch Platform"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Overwatch Root CA"),
        ]
    )
...
def generate_certs(base_dir: Path) -> None:
    """Generate all certificates for the Overwatch platform.
    ...
    server_key, server_cert = _build_server_cert(
        ca_key, ca_cert, "overwatch-server", san_dns_names
    )
